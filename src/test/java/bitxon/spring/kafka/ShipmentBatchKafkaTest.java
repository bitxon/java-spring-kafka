...
import static bitxon.spring.kafka.listener.ShipmentBatchListener.FAIL_RETRY;
...
class ShipmentBatchKafkaTest {
    ...
    @BeforeEach
    void beforeEach() {
        shipmentBatchListener.attempt.clear();
        shipmentBatchListener.repository.clear();
    }
    ...
    private void assertAttempts(Map<Integer, List<Shipment>> expectedAttempts) {
        assertThat(shipmentBatchListener.attempt.getAll()).as("Check attempts")
    ...
    private void assertProcessed(List<Shipment> expectedRecords) {
        if (expectedRecords == null || expectedRecords.isEmpty()) {
            assertThat(shipmentBatchListener.repository).as("Check entities processed")
                .isEmpty();
        } else {
            assertThat(shipmentBatchListener.repository).as("Check entities processed")
